...
An 8-page Power BI dashboard analyzing the economic, market, and policy impact of the US-China trade war and its ripple effects across equities, currencies, sectors, and inflation from 2018 through 2026.
...
- News sentiment data covers April–June 2026 only (~11 weeks) — the "Sentiment by Week" chart reflects this real window rather than a multi-year trend.
...
Source datasets from Kaggle. This project is for portfolio and educational purposes.

---

## Data Model

**Tables (11 source tables + 1 supplementary):**
`tariff_timeline`, `stock_market_reaction`, `sector_impact`, `currency_impact`, `trade_balance`, `trade_volume_annual`, `inflation_response`, `tariff_news_headlines`, `market_reaction`, `tariff_rates`, `column_metadata` — plus `vix_data`, pulled separately from FRED to fill a gap in the source data.

**Relationships:**
- `trade_balance[year]` ↔ `trade_volume_annual[year]` — **Many-to-many, both directions**. Neither table has a unique year column (trade_balance is monthly grain, trade_volume_annual has multiple rows per year across countries/indicators), so many-to-many is the correct cardinality here, not a workaround.
- Currency-to-equity correlation measures (`currency_impact` ↔ `stock_market_reaction`) deliberately **don't** use a modeled relationship — they join by explicit date-equality filters inside DAX (`CALCULATE(..., table[date] = d)`), since the two tables don't share a clean relationship key at matching grain, and a real relationship would have caused ambiguous filter propagation across the rest of the model.
...
- Pulled **VIX** historical data directly from FRED (`Web.Contents`, CSV endpoint) into a new `vix_data` table, since VIX wasn't present in the original source data.
- Removed rows with error values (e.g. FRED's `.` placeholder for non-trading days) after type conversion.
...
| `imposing_country_clean` | Column | Normalizes inconsistent country codes/names (e.g. "CHN" and "China" → one value); excludes non-country placeholders like "ALL"/"Global" |
...
| `Trade Value Affected` | Measure | Sum of estimated trade value, converted billions → trillions |
...
| Name | Type | Purpose |
|---|---|---|
| `VIX Peak` | Measure | Maximum VIX closing value across the dataset |

### `sector_impact`
| Name | Type | Purpose |
|---|---|---|
| `year` | Column | Extracted from date, for year-over-year sector comparisons |
| `Avg Annual Return by Sector` | Measure | Average daily return annualized (×252 trading days) |
...
| `Total Trade Volume` | Measure | Combined exports + imports, USA |
